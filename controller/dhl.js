...
exports.createUserOrder = async (req, res) => {
  ...
  try {
    ...
    const [user, dhl, ordersNum, response] = await Promise.all([
      userPromise,
      dhlPromise,
      ordersNumPromise,
      responsePromise,
...
exports.getUserOrders = async (req, res) => {
  const userId = req.user.user.id;
  DhlOrder.find({ user: userId, status: { $ne: "failed" } })
    .sort({ created_at: -1 })
    .then((o) => {
      res.status(200).json({
        data: o,
...
exports.getAddresses = async (req, res) => {
  axios({
    ...
    params: { type: "pickup", countryCode: "SA" },
    headers: {
      "content-type": "application/json",
      Authorization: process.env.DHL_TOKEN,
    },
